...
def new_dict(input_dict):
    ...
    for key in input_dict:
        res.update({key: input_dict[key]+5})
    return res
...
def check_same_values(input_dict):
    ...
    for key in input_dict:
        if input_dict['item1'] != input_dict[key]:
            return False
    return True
...
def combine_dicts(input_dicts):
    ...
    for d in input_dicts:
        if d['item'] in res:
            res.update({d['item']: d['amount']+res[d['item']]})
        else:
            res.update({d['item']:d['amount']})
        
        
    return res

print(combine_dicts([{'item': 'servos', 'amount': 400}, {'item': 'breadboard', 'amount': 300}, {'item': 'servos', 'amount': 750}]))